...
use crate::models::{PROJECT_STATUSES, ProjectStatus};
...
pub fn handle_key(key: KeyEvent, app: &mut App) {
    ...
    match app.mode.clone() {
        ...
        Mode::ConfirmDelete {
            kind, id, context, ..
        } => handle_confirm_delete(key, app, &kind, &id, context.as_deref()),
        ...
        Mode::AddHours {
            ...
            input,
            error,
        } => handle_add_hours(key, app, project_id, input, error),
...
fn handle_normal(key: KeyEvent, app: &mut App) {
    ...
    if app.g_pending {
        ...
        match key.code {
            ...
            KeyCode::Char('c') => {
                app.switch_view(View::Clients);
                return;
            }
            KeyCode::Char('p') => {
                app.switch_view(View::Projects);
                return;
            }
            KeyCode::Char('g') => {
                app.nav_first();
    ...
    match key.code {
        ...
        KeyCode::Char('d') if app.focus == Focus::Left => match app.active_view {
            View::Projects => {
        ...
        KeyCode::Char('r')
            if app.active_view == View::Dashboard && !app.broken_projects.is_empty() =>
        {
    ...
    match key.code {
        KeyCode::Char('y') | KeyCode::Char('Y') => {
            ...
            match kind {
                "client" => {
                    app.mode = Mode::Normal;
                    let _ = app.delete_client(&id);
                }
                "contact" => {
                    let _ = app.delete_contact(&id);
                    app.mode = Mode::ViewContacts;
                }
                "hours_entry" => {
        ...
        KeyCode::Char('n') | KeyCode::Char('N') | KeyCode::Esc => {
            app.mode = match kind {
                ...
                "hours_entry" => Mode::ViewHours {
                    project_id: context.unwrap_or("").to_string(),
                },
                _ => Mode::Normal,
            };
        }
            ...
            if form.focused == 1 {
                ...
                if !typed.is_empty()
                    && let Some(c) = app
                        .clients
                        .iter()
                        .find(|c| c.name.to_lowercase().starts_with(&typed))
                {
...
fn handle_add_hours(
    ...
    if save {
        match input.trim().parse::<f64>() {
            Ok(h) if h > 0.0 => match app.save_hours(&project_id, h) {
                Ok(_) => {
                    app.mode = Mode::ViewHours { project_id };
                    app.focus = Focus::Right;
                }
...
fn handle_view_hours(key: KeyEvent, app: &mut App, project_id: String) {
    ...
    match key.code {
        ...
        KeyCode::Char('e') => {
            let data = app.selected_hours_entry(&project_id).map(|e| {
                let h = e
                    .hours
...
fn handle_edit_hours(
    ...
    if save {
        match hours_input.trim().parse::<f64>() {
            Ok(h) if h > 0.0 => match app.edit_hours_entry(&activity_id, h, date_input.trim()) {
                Ok(_) => {
                    app.mode = Mode::ViewHours { project_id };
                }
                Err(e) => {
    ...
    match key.code {
        ...
        KeyCode::Char(c)
            if focused == 1 && (c.is_ascii_digit() || (c == '.' && !hours_input.contains('.'))) =>
        {
            hours_input.push(c);
            app.mode = Mode::EditHours {
                project_id,
