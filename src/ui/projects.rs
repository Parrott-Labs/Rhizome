...
fn render_project_detail(frame: &mut Frame, p: &Project, area: Rect) {
    ...
    // Burn label
    frame.render_widget(Paragraph::new(Span::styled("Burn", fg(SUBTLE))), layout[6]);
...
fn render_project_kv(frame: &mut Frame, p: &Project, area: Rect) {
    ...
    if let Some(b) = p.budget_hours {
        let unit = if p.budget_kind == "monthly" {
            "h / mo"
        } else {
            "h total"
        };
...
fn render_burn_bars(frame: &mut Frame, p: &Project, area: Rect) {
    ...
    if let Some(budget) = p.budget_hours {
        let frac = if budget > 0.0 {
            p.spent_hours / budget
        } else {
            0.0
        };
...
fn render_new_project_form(
    ...
    for (i, field) in form.fields.iter().enumerate() {
        ...
        // Budget kind selector — cycling pills (field 4)
            ...
            let mut spans = vec![Span::styled(format!("  {:<LW$}", field.label), label_style)];
        ...
        // Client field gets ghost-text completion
            ...
            let visible_typed: String = typed_chars[typed_chars.len() - typed_visible..]
                .iter()
                .collect();
...
fn render_hours_list(
    ...
    let items: Vec<ListItem> = entries
        ...
        .map(|(i, e)| {
            ...
            let line = Line::from(vec![
                ...
                Span::styled(
                    format!("{:>6.1}h", h),
                    if selected { bold_fg(BRAND) } else { fg(CYAN) },
                ),
                Span::raw("  "),
    ...
    if items.is_empty() {
        frame.render_widget(
            Paragraph::new(Span::styled(
                "no hours logged yet · press a to add",
                fg(MUTED),
            )),
            layout[2],
        );
...
fn render_milestones(frame: &mut Frame, milestones: &[Milestone], area: Rect) {
    ...
    let lines: Vec<Line> = sorted
        ...
        .map(|m| {
            ...
            let text_style = if m.is_done() {
                fg(SUBTLE)
            } else if m.done_at.is_none() {
            ...
            let title_style = if !m.is_done() && {
                sorted
                    .iter()
                    .all(|prev| prev.ord < m.ord && !prev.is_done() || prev.ord >= m.ord)
            } {
                fg(ACCENT)
            } else {
                text_style
