...
fn render_broken_warning(frame: &mut Frame, app: &App, area: Rect) {
    if app.broken_projects.is_empty() { return; }

    let mut spans: Vec<Span> = vec![
        Span::styled("✗ broken  ", Style::default().fg(RED).add_modifier(Modifier::BOLD)),
    ];
    for (i, bp) in app.broken_projects.iter().enumerate() {
        if i > 0 { spans.push(Span::styled("  ·  ", fg(MUTED))); }
        spans.push(Span::styled(bp.name.clone(), fg(ACCENT)));
        if !bp.bad_status.is_empty() {
            spans.push(Span::styled(format!(" (\"{}\" unrecognised)", bp.bad_status), fg(RED)));
        }
    }
...
fn render_followup_warning(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let mut spans: Vec<Span> = vec![
        Span::styled("! follow-up  ", Style::default().fg(AMBER).add_modifier(Modifier::BOLD)),
    ];
...
fn render_stats(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let s = &app.stats;
    let client_delta  = if s.clients_this_quarter > 0 { format!("+{} this quarter", s.clients_this_quarter) } else { String::new() };
    let project_delta = if s.total_projects > 0       { format!("of {} total", s.total_projects) }           else { String::new() };
    let attn_delta    = if s.attention_count > 0 { "need follow-up".into() } else { String::new() };
    let hours_val     = if s.hours_this_week > 0.0 { format!("{:.1}h", s.hours_this_week) } else { "—".into() };
    render_stat_cell(frame, cells[0], "CLIENTS",         &s.total_clients.to_string(), &client_delta,  GREEN,  false);
    render_stat_cell(frame, cells[1], "ACTIVE PROJECTS", &s.active_projects.to_string(), &project_delta, SUBTLE, false);
    render_stat_cell(frame, cells[2], "HOURS LOGGED",    &hours_val,                   "",             SUBTLE, false);
    render_stat_cell(frame, cells[3], "ATTENTION",       &s.attention_count.to_string(), &attn_delta,  AMBER,  s.attention_count > 0);
}
...
fn render_stat_cell(
    ...
    let lines = vec![
        Line::from(Span::styled(label.to_string(), Style::default().fg(SUBTLE).add_modifier(Modifier::DIM))),
        Line::from(Span::styled(value.to_string(), Style::default().fg(val_color).add_modifier(Modifier::BOLD))),
...
fn render_left(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let items: Vec<ListItem> = app.projects.iter().take(5).enumerate().map(|(i, p)| {
        project_list_item(i, p, i == selected)
    }).collect();
...
fn render_hours_bars(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let mut client_hours: Vec<(String, f64)> = app.clients.iter().map(|c| {
        let h: f64 = app.projects.iter()
            .filter(|p| p.client_id == c.id && p.status != ProjectStatus::Archived)
            .map(|p| p.spent_hours)
            .sum();
        (c.name.clone(), h)
    }).filter(|(_, h)| *h > 0.0).collect();
    client_hours.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap());

    let max_h = client_hours.first().map(|(_, h)| *h).unwrap_or(1.0);
    let bar_w = area.width as usize;

    let lines: Vec<Line> = client_hours.iter().take(area.height as usize).map(|(name, hours)| {
...
fn build_graph_lines(clients: &[Client], projects: &[Project]) -> Vec<Line<'static>> {
    ...
    for (ci, client) in clients.iter().enumerate() {
        ...
        let client_projects: Vec<&Project> = projects.iter()
...
fn render_next_up(frame: &mut Frame, app: &App, area: Rect) {
    ...
    for project in &app.projects {
        if matches!(project.status, ProjectStatus::Archived | ProjectStatus::Lead) { continue; }
        for ms in &project.milestones {
            if !ms.is_done() {
                ...
                if upcoming.len() >= 4 { break; }
            }
        }
        if upcoming.len() >= 4 { break; }
    }

    let lines: Vec<Line> = upcoming.iter().enumerate().map(|(i, (label, due))| {
        let (sym, sym_color) = if i == 0 { ("→", BLUE) } else { ("·", SUBTLE) };
        Line::from(vec![
