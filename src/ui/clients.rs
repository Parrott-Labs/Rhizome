...
fn render_client_list(frame: &mut Frame, app: &App, area: Rect) {
    ...
    frame.render_widget(
        Paragraph::new(Line::from(vec![
            ...
            Span::styled(format!("{:<22}", "NAME"),          Style::default().fg(SUBTLE).add_modifier(Modifier::DIM)),
            Span::styled(format!("{:<16}", "CITY"),           Style::default().fg(SUBTLE).add_modifier(Modifier::DIM)),
            Span::styled(format!("{:<5}", "PROJ"),           Style::default().fg(SUBTLE).add_modifier(Modifier::DIM)),
            Span::styled(format!("{:<12}", "LAST CONTACT"),  Style::default().fg(SUBTLE).add_modifier(Modifier::DIM)),
    ...
    let items: Vec<ListItem> = app.clients.iter().enumerate()
...
fn render_client_detail(
    ...
    // "CLIENT" label
    frame.render_widget(
        Paragraph::new(Span::styled("CLIENT", Style::default().fg(SUBTLE).add_modifier(Modifier::DIM))),
        layout[0],
    );
...
fn render_kv_block(frame: &mut Frame, client: &Client, area: Rect) {
    ...
    lines.push(kv("domain",   client.domain.clone().unwrap_or_default(), ACCENT));
...
fn render_mini_projects(frame: &mut Frame, projects: &[&crate::models::Project], area: Rect) {
    let lines: Vec<Line> = projects.iter().take(area.height as usize).map(|p| {
        Line::from(vec![
...
fn render_contacts(frame: &mut Frame, contacts: &[&ContactMoment], area: Rect) {
    if contacts.is_empty() {
        frame.render_widget(
            Paragraph::new(Span::styled("  no contacts yet · press c to log one", fg(MUTED))),
            area,
        );
        return;
    }

    let lines: Vec<Line> = contacts.iter().take(area.height as usize).map(|c| {
        Line::from(vec![
...
fn render_new_client_form(frame: &mut Frame, form: &NewClientForm, area: Rect) {
    ...
    let title = if form.edit_id.is_some() { "EDIT CLIENT" } else { "NEW CLIENT" };
...
fn render_contact_list_view(
    ...
    frame.render_widget(
        Paragraph::new(Line::from(vec![
            Span::styled("CONTACTS · ", Style::default().fg(SUBTLE).add_modifier(Modifier::DIM)),
    ...
    if contacts.is_empty() {
        ...
        let items: Vec<ListItem> = contacts.iter().enumerate().map(|(i, c)| {
            ...
            let edge = if sel { Span::styled("│", fg(BRAND)) } else { Span::raw(" ") };
            Line::from(vec![
                edge,
                Span::raw(" "),
                Span::styled(c.date.clone(),                   if sel { bold_fg(ACCENT) } else { fg(SUBTLE) }),
                Span::raw("   "),
                Span::styled(c.kind_symbol().to_string(),      fg(c.kind_color())),
                Span::raw(" "),
                Span::styled(format!("{:<8}", &c.kind),        fg(c.kind_color())),
            ...
            ListItem::new(Line::from(vec![
                if sel { Span::styled("│", fg(BRAND)) } else { Span::raw(" ") },
                Span::raw(" "),
                Span::styled(c.date.clone(),              if sel { bold_fg(ACCENT) } else { fg(SUBTLE) }),
                Span::raw("   "),
                Span::styled(c.kind_symbol().to_string(), fg(c.kind_color())),
...
fn render_new_contact_form(frame: &mut Frame, form: &NewContactForm, area: Rect) {
    ...
    lines.push(Line::from(vec![
        Span::styled("NEW CONTACT · ", Style::default().fg(SUBTLE).add_modifier(Modifier::DIM)),
    ...
    for (i, field) in form.fields.iter().enumerate() {
        ...
        let line = if i == 1 {
            let spans: Vec<Span> = CONTACT_KINDS.iter().map(|&k| {
